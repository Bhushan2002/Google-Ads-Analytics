...
exports.disconnectAccount = exports.checkConnection = exports.getStoredRefreshToken = exports.handleGoogleCallback = exports.getAuthUrl = void 0;
const google_auth_library_1 = require("google-auth-library");
const GoogleAccount_1 = require("../models/GoogleAccount");
    ...
    try {
        ...
        if (tokens.refresh_token) {
            if ((0, database_1.isDatabaseConnected)()) {
                ...
                await GoogleAccount_1.GoogleAccount.findOneAndUpdate({ userId: 'test-user-1' }, { refreshToken: tokens.refresh_token, connectedAt: new Date() }, { upsert: true, new: true });
            ...
            const existing = await (0, exports.getStoredRefreshToken)();
            if (existing) {
                console.log('No new refresh token returned, but existing token found — OK.');
                return res.redirect('http://localhost:3000/?status=success');
            }
            const msg = encodeURIComponent('No refresh token received. Please disconnect and reconnect your Google account.');
            return res.redirect(`http://localhost:3000/?status=error&message=${msg}`);
        }
    }
    catch (err) {
...
const getStoredRefreshToken = async () => {
    if ((0, database_1.isDatabaseConnected)()) {
        const account = await GoogleAccount_1.GoogleAccount.findOne({ userId: 'test-user-1' });
...
const checkConnection = async (req, res) => {
    try {
        const token = await (0, exports.getStoredRefreshToken)();
        if (token) {
...
const disconnectAccount = async (req, res) => {
    try {
        ...
        if ((0, database_1.isDatabaseConnected)()) {
            await GoogleAccount_1.GoogleAccount.deleteOne({ userId: 'test-user-1' });
        }
